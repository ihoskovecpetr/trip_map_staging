...
const getFormatedPriceString = (amount, currency) => {
    return amount ? `| ${getFormattedPrice({ amount: amount, currency: currency })}` : ''
}

export default function MapContainer({ map, snapMapInstance, addZoom, subtractZoom, mapTitles }) {
    ...
    useEffect(() => {
        setAnchorEl(!seenPopup ? document.getElementById('full_screen_button') : null)
    }, [])
...
const ZoomBtn = styled.div`
    ...
    border-radius: ${({ isLeft }) =>
        isLeft ? `${BNT_RADIUS}px 0 0 ${BNT_RADIUS}px` : `0 ${BNT_RADIUS}px ${BNT_RADIUS}px 0`};
