...
export default function CheckoutItems({ dataPrintful, activeMapStyleName }) {
    ...
    const productDescription = t(getVariantObject(productRedux.variantId)?.frameName)

    const dataPrintfulVariant = dataPrintful && dataPrintful[productRedux.variantId]

    const priceAlgorithm = getPriceAlgorithm()

    const priceOfDelivery = priceAlgorithm.getPriceOfDelivery(productRedux.variantId, dataPrintful)

    const priceWithDelivery = dataPrintful?.[productRedux.variantId]?.priceWithDeliveryAndProfit.netPrice ?? 0

    const priceDiscounted = priceAlgorithm.getDiscountedPrice(priceWithDelivery, discount.code)
        ...
        <Container>
            <CheckoutSection>
                <LineTextContainer>
                    <BoldText>
                        {t('checkout.haveCode')}
                        {/* <input type="checkbox" checked={discountInputOpen} onChange={changeDiscountInput} /> */}
                    </BoldText>
                </LineTextContainer>
                <StyledDiscountCode>
                    {/* {discountInputOpen && ( */}
            ...
            <CheckoutSection>
                ...
                {/* <StyledPriceSpan>
                    <StyledPrice isCrossed={discount.codeAccepted && !checkingCode}>
                        {priceWithDelivery
          ...
          <StyledImage src={deliveryTruck} />
                ...
                <StyledPriceSpan>{getFormattedPrice({ amount: priceOfDelivery.netPrice, currency })}</StyledPriceSpan>
            </CheckoutSection>
...
const CheckoutSection = styled.span`
    display: flex;
    width: 100%;
    margin: 10px 0px;
...
const BoldText = styled.p`
    ...
    margin: 0;
    font-size: ${({ isLarge }) => (isLarge ? fontSize('sm') : fontSize('xs'))};
    font-weight: ${fontWeight('bold')};
    text-align: left;
    text-decoration: ${({ isCrossed }) => (isCrossed ? 'line-through' : 'unset')};
    color: ${({ isCrossed }) => (isCrossed ? color('muted') : 'unset')};
    margin-bottom: ${({ isCrossed }) => (isCrossed ? '-5px' : '0')};
`
...
const StyledPriceSpan = styled.div`
    font-size: ${({ isLargePrice }) => (isLargePrice ? fontSize('sm') : fontSize('xs'))};
    font-weight: ${fontWeight('bold')};
    text-align: right;
    flex-grow: 1;
    flex-basis: 35%;
