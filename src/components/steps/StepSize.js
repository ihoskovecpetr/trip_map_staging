...
export default function StepSize({ index }) {
    ...
    const findVariant = newSize => {
        const currentFrameName = VARIANTS_PRINTFUL.find(variant => variant.id === productRedux.variantId)?.frameName
    ...
    const findNewSizeSameOrientation = newSizeCode =>
        SIZES.find(
            size =>
                size.acceptableSizes.includes(newSizeCode) && size.orientation === productRedux.sizeObject.orientation
        )
    ...
    const setNewSize = sizeCode => {
        ...
        dispatch(
            setProductAction({
                ...
                shippingCode: newVariant.shipping[LANGUAGE_CURRENCY_TABLE[locale]].codeCZ
            })
        )
    }
    ...
    const getNoFrameVariantForSize = sizeName => {
        return VARIANTS_PRINTFUL.find(variantObj => {
            if (variantObj.frameName === FRAME_OPTION_NAMES.NO_FRAME && variantObj.sizeName === sizeName) {
                return true
            }
            ...
            <ContainerSizes>
                {variantsPrintfulForSize.length > 0 &&
                    ...
                    arrSingleSizeNames.map((sizeNameLocal, index) => {
                        const sizeObject = SIZES.find(size => size.code === sizeNameLocal)

                        const noFrameVariant = getNoFrameVariantForSize(sizeNameLocal)

                        const printFulVariantObj = dataPrintful?.[noFrameVariant.id]
                        const priceWithDeliveryNoFrame = printFulVariantObj?.priceWithDeliveryAndProfit.netPrice ?? 0

                        const isDisabledBtn =
                            (sizeNameLocal === SIZE_NAMES['61X91cm'] && (isMobile || !isLargeSizeCapable)) ||
                        ...
                        return (
                            <MainItem key={index}>
                                <StyledItem
                                    active={productRedux.sizeObject.acceptableSizes.includes(sizeNameLocal)}
                                    key={index}
                                ...
                                >
                                    <SizeIconWrap>
                                        {getSizeIcon(
                                            sizeObject?.code,
                                            productRedux.sizeObject.acceptableSizes.includes(sizeNameLocal)
                                        )}
                                    </SizeIconWrap>
                                    <SizeName active={productRedux.sizeObject.acceptableSizes.includes(sizeNameLocal)}>
                                        {sizeObject?.name}
                                    </SizeName>
                                </StyledItem>
                                <StyledPriceP active={productRedux.sizeObject.acceptableSizes.includes(sizeNameLocal)}>
                                    {`(${getFormattedPrice({
                                        amount: priceWithDeliveryNoFrame,
                                        currency
                                    })})`}
                                </StyledPriceP>
                                <DevicesItem>{isDisabledBtn && <StyledDevicesImg src={disabledAndroid} />}</DevicesItem>
                            </MainItem>
                        )
                    })}
            </ContainerSizes>
