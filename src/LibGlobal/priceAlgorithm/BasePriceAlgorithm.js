...
module.exports = class BasePriceAlgorithm {
    ...
    add(values) {
        ...
        const targetValue = values.reduce((acc, cur) => acc.plus(cur), initialValue)
    ...
    subtract(valuesArr) {
        ...
        const targetValue = restValues.reduce((acc, cur) => acc.minus(cur), initialValue)
    ...
    round(value, precision = this.roundingPrecision) {
        ...
        return Math.round((value + Number.EPSILON) * precisionMultiple) / precisionMultiple
    }
