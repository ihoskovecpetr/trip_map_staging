...
import { setNewTitle, setNewSubtitle, setMapZoomAction } from 'redux/order/actions'
...
const resizeFrameDiv = ({ productRef, mapAvailSpaceRef }) => {
    ...
    if (productRef.current.sizeObject.ratio > 1) {
        frameWidthKoefficient = OUTSIDE_FRAME_CM / productRef.current.sizeObject.height
    } else {
        frameWidthKoefficient = OUTSIDE_FRAME_CM / productRef.current.sizeObject.width
    }
...
const resizeInputs = ({ mapTitles, saveTitlesValue, mapHeight, mapWidth, layout, activeMapStyleName, productRef }) => {
...
export default function StudioRootContainer() {
    ...
    const { height: mapWrapperHeight, width: mapWrapperWidth } = useElementDimensions('map_wrap_id')
    ...
    const { paddingWidth, layoutObj, baseLongSize } = getCenteringLayoutDimensions({
        product: productRef.current,
    ...
    useEffect(() => {
        if (mapInstance) {
            ...
            const mapWrapDivElement = document.getElementsByClassName('mapboxgl-map')[0]
            trueMapCanvasElement = document.getElementsByClassName('mapboxgl-canvas')[0]

            const mapCanvasWrapElement = document.getElementsByClassName('mapboxgl-canvas-container')[0]
    ...
    }, [productRedux, activeLayoutRedux, mapAvailSpaceHeight, mapAvailSpaceWidth, JSON.stringify(mapTitles)])
    ...
    return (
        <StyledTrueContainer>
            {screenHeight && (
                <StyledSection isMobile={isMobile} headerHeight={headerHeight} screenHeight={screenHeight}>
                    <ContainerBox headerHeight={isMobile ? 0 : headerHeight} isMobile={isMobile}>
                        <CanvasBox>
                            <MapContainer
                                map={mapInstance}
...
const StyledSection = styled.section`
    /* margin-top: ${({ headerHeight, isMobile }) => (isMobile ? 0 : `${headerHeight}px`)}; */
`
