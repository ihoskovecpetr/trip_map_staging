...
import ProductFramedPNG from 'assets/packages/sf.png'
import ProductNoFramePNG from 'assets/packages/iceland.png'
...
export default function PackagesOptions() {
    ...
    return (
        <section id="packages" sx={styles.packages}>
            <Container>
                <SectionHeader title={t('packages.title')} slogan={t('packages.subtitle')} isWhite={true} />
                <Flex
                    sx={{
                    ...
                    {packages.map(packageData => (
                        <PackageCard
                            ...
                            priceFresh={`${getFormattedPrice({
                                amount: dataPrintful?.[packageData.variantId]?.priceWithDeliveryAndProfit.netPrice ?? 0,
                                currency: currency
                            })}`}
